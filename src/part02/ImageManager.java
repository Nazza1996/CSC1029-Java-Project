...
public class ImageManager {
    ...
    public ImageRecord[] images = new ImageRecord[collection.size()];
    ...
    public ImageAlbum searchTitle(String str) {
        ...
        for (ImageRecord imageRecord : collection) {
            if (imageRecord.getTitle().toLowerCase().contains(str.toLowerCase())) { // Set to lowercase to avoid case sensitivity
                album.add(imageRecord);
            }
        }

        if (album.size() > 0) {
            return new ImageAlbum(album).dateSort();
        } else {
            return null;
        }
    }
    ...
    public ImageAlbum searchDescription(String str) {
        ArrayList<ImageRecord> album = new ArrayList<ImageRecord>();
        for (ImageRecord imageRecord : collection) {
            if (imageRecord.getDescription().toLowerCase().contains(str.toLowerCase())) { // Set to lowercase to avoid case sensitivity
                album.add(imageRecord);
            }
        }
    ...
    public ImageAlbum searchDates(LocalDate start, LocalDate end) {
        ArrayList<ImageRecord> album = new ArrayList<ImageRecord>();
        for (ImageRecord imageRecord : collection) {
            if (imageRecord.getDateTaken().isAfter(start.minusDays(1)) && imageRecord.getDateTaken().isBefore(end.plusDays(1))) {
                album.add(imageRecord);
            }
        }
